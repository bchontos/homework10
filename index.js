...
async function viewEmployeesByDepartment() {
  ...
  const deprtmentChoices = departments.map(({ id, name }) => ({
    name: name,
    value: id
  }));
  ...
  const { departmentId } = await prompt([
    {
      type: "list",
      name: "departmentId",
      message: "Which department would you like to see eployees for?",
  ...
  const { employeeId } = await prompt([
    {
      type: "list",
      name: "employeeId",
      message: "Which employee's manager needs updated?",
      choices: employeeChoices
    }
  ]);
  ...
  const { managerId } = await prompt([
    {
      type: "list",
      name: "managerId",
      message: "Which employee do you wnat to set as manager for selected employee?",
      choices: managerChoices
    }
  ]);
